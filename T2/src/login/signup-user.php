<?php
  ...
  if (!empty($_POST['nombre']) && !empty($_POST['apellido']) && !empty($_POST['user']) && !empty($_POST['password'])) {
    ...
    //Agregamos en Usuarios
    $username = "'".$_POST['user']."'";
    $password = "'".$_POST['password']."'";
    $values = "('".$ID."',".$username.",".$password.")";
    $stmt = "INSERT INTO Usuarios(ID_persona,nombre_usuario,password) VALUES ".$values;
    $query2 = mysqli_query($conn,$stmt);

    if ($query && $query2) {
      $message = "Usuario registrado correctamente. Dirigase a iniciar sesion";
    }
    else {
      $message = "Usuario no registrado, revise datos ingresados";
    }
  }
?>
...
<html>
  ...
  <body>
    ...
    <span>Si tiene cuenta: <a href="login-user.php">iniciar sesion como usuario</a></span><br>
